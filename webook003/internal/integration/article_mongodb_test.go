	...
	testCases := []struct {
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				defer cancel()
				// 模拟已经存在的帖子
				_, err := s.col.InsertOne(ctx, &dao.IsDaoArticle{
					Id:       2,
					Title:    "我的标题",
					Content:  "我的内容",
					Ctime:    456,
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				_, err := s.col.InsertOne(ctx, &art)
				assert.NoError(t, err)
				part := dao.PublishedArticle(dao.IsDaoArticle{
					Id:       4,
					Title:    "我的标题",
					Content:  "我的内容",
					Ctime:    456,
