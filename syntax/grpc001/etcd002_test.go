...
func TestEtcd002(t *testing.T) {
	suite.Run(t, new(EtcdTestSuite002))
}

type EtcdTestSuite002 struct {
	suite.Suite
	cli *etcdv3.Client
}

func (s *EtcdTestSuite002) SetupSuite() {
...
func (s *EtcdTestSuite002) TestClient() {
	t := s.T()
...
func (s *EtcdTestSuite002) TestServer() {
	t := s.T()
	...
	defer cancel()
	//addr := "127.0.0.1:8090"
	addr := netx.GetOutboundIP() + ":8090" // 获取本机ip 互联网视角
	key := "service/user/" + addr
	t.Log(addr, key)
	listenS, err := net.Listen("tcp", ":8090")
	require.NoError(s.T(), err)
